...
@Pipe({name : 'logsFilter', pure : false})
export class LogsFilterPipe implements PipeTransform {
  transform(logs: Publish<any>[], filterLogName: string,
    ...
    return logs
        .filter(log => {
          return (filterLogName === '' ||
                  log.name.indexOf(filterLogName) !== -1) &&
                 (filterLogText === '' ||
                  JSON.stringify(log.data).indexOf(filterLogText) !== -1);
        })
        .reverse();
  }
}
